        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            if (Regex.IsMatch(busqueda, "^[a-zA-Z]"))
            {
                ...
                dbConn.llenarDGV(dgvListadoPersonalAdmin, "select concat(de.[primerNombre],' ', de.[segundoNombre], ' ', de.[primerApellido], ' ', de.[segundoApellido]) as 'Nombre del medico'," +
                    ...
                    " inner join[dbo].[detalleCargos] det on de.[identidadPersona] = det.[identidadEmpleado] and det.[idCargoAsociado] = 2 and [primerNombre] = '" + busqueda + "'");
            ...
            else if ((Regex.IsMatch(busqueda, @"^\d+$")))
            {
                ...
                if (Regex.IsMatch(busqueda, "[0-9]{13,13}"))
                {
                    ...
                    "de.[identidadPersona] as 'Identidad', de.[numeroTelefono] as 'Numero teléfonico', de.[correoElectronico] as 'Correo'," +
                    "de.[fechaNacimiento] as 'Fecha Nacimiento', de.[estadoEmpleado] as 'Estado' from[dbo].[datosEmpleados] de" +
                    " inner join[dbo].[detalleCargos] det on de.[identidadPersona] = det.[identidadEmpleado] and det.[idCargoAsociado] = 2 and [identidadPersona] =" + busqueda);
                    txtBusqueda.Clear();
                }
                else
                {
